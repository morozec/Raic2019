	...
	while (counter < MAX_SIMULATIONS)
	{
		...
		else
		{
...

			{
				...
				isMyClosestUnit && isDangerousZone && isEnemyShootEarlier) //или если € близко, а он стрел€ет раньше
			{
				needStop = true;
				action.jump = false;
				action.jumpDown = false;
	...
	const Game& game, UnitAction& action, bool isHealing)
{
	...
	if (canShootingTick == 1 && movingTime < tickTime)//выстрел в рамках тика 0-1
	{
		...
		const auto enemyShootingPositions = enemyPositions[movingTime < tickTime/2 ? 0 : 1]; //позиции врага, начина€ с тика выстрела
	...
	while (canShootingTick + addShootingSimulations < MAX_SIMULATIONS)
	{
		...
		const auto enemyShootingPositions = enemyPositions[shootingTick]; //позиции врага, начина€ с тика выстрела
		double minAngle = INT_MAX;
		double maxAngle = -INT_MAX;
		for (const auto& ep : enemyShootingPositions)
		{
	...
	if (maxShootingProbability >= NOT_BAD_SHOOTING_PROBABILITY) 
	{
		while (true)
		{
			...
			while (canShootingTick + addShootingSimulations2 < canShootingTick + okAddShootingSimulations - shootDelay)
			{
				...
				//позици€, откуда произойдет выстрел
				...
				const auto enemyShootingPositions = enemyPositions[bestShootingTick]; //позиции врага, начина€ с тика выстрела
	...
	//else if (mePositions.size() == 1 && maxShootingProbability > TOLERANCE)// дальше € уже не пойду
	//{
	...
	bool isHealing)
{
	...
	if (runawayDirection != GoNONE)//потом придетс€ убегать
	{
                                 ...
                                 Debug& debug)
{
	if (!strategy_.isInit)
	{
		...
		const auto Z_SIZE = maxJumpTiles + 2; //+1 - на падение, +1 - на сто€ние
	...
	if (isMonkeyMode && (
		...
		!Simulator::isUnitOnAir(unit.position, unit.size, unit.id, game))) //€ оказалс€ на земле
	{
