	...
	for (const auto& lb : game.lootBoxes)
	{
		if (std::dynamic_pointer_cast<Item::HealthPack>(lb.item))
		{
			...
			if ((static_cast<int>(lb.position.x) - static_cast<int>(enemy.position.x)) *
				(static_cast<int>(me.position.x) - static_cast<int>(enemy.position.x)) < 0) continue;
	...
	const Game& game, UnitAction& action, bool isHealing)
{
	...
	//const auto& meShootingPosition = mePositions.second;//позиция, откуда произойдет выстрел
