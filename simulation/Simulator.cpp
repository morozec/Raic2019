	...
	const Game& game)
{
		...
		for (int j = 0; j < microTicksCount; ++j)
		{
			...
			{		
				...
				if (isPlatformCross)//проверим пересечение с платформой
				{
					...
					{
						if (size_t(y) > size_t(nextY))//€ до этого был тайлом выше
						{
			...
			//		y = jumpStoppedCur ?
			//			nextY :
			//			trunc(nextY); //после падени€ встаем ровно на тайл
	...
	else if (isPlatformCross)
	{
		for (int j = 0; j < microTicksCount; ++j)
		{
			...
			{
				if (size_t(y) > size_t(yj))//€ до этого был тайлом выше
				{
					jumpState.canJump = true;
					jumpState.speed = game.properties.unitJumpSpeed;
					jumpState.maxTime = game.properties.unitJumpTime;
				...
				else //платформа не повли€ет на мое движение
				{
					return { nextX, nextY };
				}
			}
	...
	else if (bottomUnit != nullptr)
	{
		for (int j = 0; j < microTicksCount; ++j)
		{
			...
			{
				if (y > bottomUnit->position.y + bottomUnit->size.y) //до этого € был сверху
					{
						jumpState.canJump = true;
						jumpState.speed = game.properties.unitJumpSpeed;
						jumpState.maxTime = game.properties.unitJumpTime;
				...
				else //врезаемс€ в вертикальную сторону юнита
					{
						return {
							velocityX > 0 ? 
		...
		//здесь может немного не сойтись, если врежетс€ на последнем микротике (который мы не считаем)
		//
